...
function renderCoinSwitcher() {
  ...
  return `<nav class="coin-switcher" aria-label="${escapeAttribute(t('coinSwitcher'))}">${coins.map((coin) => {
    ...
    return `<a class="coin-chip ${active ? 'active' : ''}" href="/${encodeURIComponent(coin.id)}" data-route ${active ? 'aria-current="page"' : ''}><span class="coin-chip-dot" style="background:${escapeAttribute(coin.color)}" aria-hidden="true"></span>${escapeHtml(coin.name)}<small class="mono">${escapeHtml(coin.symbol)}</small></a>`;
  }).join('')}</nav>`;
}
