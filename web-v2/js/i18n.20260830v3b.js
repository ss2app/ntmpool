...
const messages = {
  zh: {
    // ── v3b 阶段②「换骨」新增（头部币种切换器 / 地址搜索 / 首页账本行 / 仪表盘 band） ──
    coinSwitcher: '矿池切换', headerSearchLabel: '按矿工地址查询', headerSearchPlaceholder: '矿工地址',
    searchSearching: '正在查询…', searchNoMatch: '这个地址在本站各池都没有记录，请核对后重试。', searchFoundIn: '在 {coin} 找到该地址',
    homeTitle: '矿池总览', homeSubtitle: '自研锄头 + 透明矿池。选一个池，下载它对应的锄头，一条命令开挖；页面上每个数字都直读矿池 API。',
    poolsOnlineCount: '{count} 个矿池在线', minersUnit: '矿工', rigsUnit: '矿机', blocksUnit: '累计爆块',
    spark24h: '24h 池算力', sparkNoSamples: '24h 无样本', feeAndScheme: '池费 · 结算', enterPool: '进入',
    stepsTitle: '三步开挖',
    homeStep1: '选一个池', homeStep1Body: '上面任选。每个池页的「接入」标签列出全部接入点和结算规则。',
    homeStep2: '下载它对应的锄头', homeStep2Body: '每个币对应一个推荐锄头，抽水比例逐个明码公示。',
    homeStep3: '一条命令', homeStep3Body: '把钱包地址填进去就能跑。Windows 双击 .bat，Linux / HiveOS 用常驻脚本——生成器在每个锄头页的「一键上手」里。',
    feeDisclosure: '锄头抽水公示', thirdPartyOfficial: '第三方官方锄头',
    poolShareValue: '池占比 {value}', networkFieldMissing: '节点 API 未提供该字段',
    minersWindowNote: '按 10 分钟算力窗口去重', rigsConnNote: '当前 stratum 连接数', sharesPerSecondShort: 'shares/s',
    orphanRate: '孤块率', confirmedVsOrphaned: '已确认 / 孤块', directPaidTotal: '已直付累计', blockTimeline: '爆块时间线',
    lastNBlocks: '最近 {count} 块', viewAll: '查看全部', snapshotAt: '快照 {time}', payoutDirectShort: 'coinbase 直付', payoutMethod: '打款方式', confirmationsShort: '确认数',
    lookupSearchHint: '搜索命中后会自动跳到该币的「我的矿机」并查询。',
    skip: '跳到主要内容',
  ...
  en: {
    // ── v3b stage 2 additions (coin switcher / address search / home ledger rows / dashboard band) ──
    coinSwitcher: 'Switch pool', headerSearchLabel: 'Look up a miner address', headerSearchPlaceholder: 'Miner address',
    searchSearching: 'Searching…', searchNoMatch: 'No pool on this site has a record for that address — please check it and try again.', searchFoundIn: 'Found on {coin}',
    homeTitle: 'Pool overview', homeSubtitle: 'In-house miners + transparent pools. Pick a pool, download its miner, start with one command. Every number on this page is read straight from the pool API.',
    poolsOnlineCount: '{count} pools online', minersUnit: 'miners', rigsUnit: 'rigs', blocksUnit: 'blocks found',
    spark24h: '24h pool hashrate', sparkNoSamples: 'no samples in 24h', feeAndScheme: 'Fee · scheme', enterPool: 'Open',
    stepsTitle: 'Three steps to mine',
    homeStep1: 'Pick a pool', homeStep1Body: 'Any of the above. Each pool page has a Connect tab listing every endpoint and the payout rules.',
    homeStep2: 'Download its miner', homeStep2Body: 'Each coin has one recommended miner, with its dev fee published openly.',
    homeStep3: 'One command', homeStep3Body: 'Paste your wallet address and run. Windows: double-click the .bat; Linux / HiveOS: the keep-alive script — the generator lives on each miner page under Quick start.',
    feeDisclosure: 'Miner dev fees', thirdPartyOfficial: 'Official third-party miner',
    poolShareValue: 'Pool share {value}', networkFieldMissing: 'Not provided by the node API',
    minersWindowNote: 'Unique addresses in the 10-min window', rigsConnNote: 'Current stratum connections', sharesPerSecondShort: 'shares/s',
    orphanRate: 'Orphan rate', confirmedVsOrphaned: 'Confirmed / orphaned', directPaidTotal: 'Direct-paid total', blockTimeline: 'Block timeline',
    lastNBlocks: 'Last {count} blocks', viewAll: 'View all', snapshotAt: 'Snapshot {time}', payoutDirectShort: 'coinbase direct-pay', payoutMethod: 'Payout', confirmationsShort: 'Confirmations',
    lookupSearchHint: 'A hit jumps straight to that coin’s My rigs tab and runs the query.',
