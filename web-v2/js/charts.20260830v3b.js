...
// v3b（阶段②）：首页账本行的 24h 迷你曲线。无坐标轴、无交互，只画趋势 + 端点。
export function renderSparkline(container, samples, options) {
  const points = preparePerformance(samples, options.bucketMs, 'poolHashrate');
  ...
  const draw = () => {
    if (destroyed) return;
    clearContainer(container);
    const width = Math.max(80, Math.round(container.clientWidth || 160));
    const height = Math.max(28, Math.round(container.clientHeight || 40));
    const minTime = points[0].time;
    const maxTime = points[points.length - 1].time;
    const maxValue = Math.max(...points.map((point) => point.value), 0) || 1;
    const x = (time) => 2 + ((time - minTime) / Math.max(1, maxTime - minTime)) * (width - 8);
    const y = (value) => 4 + (1 - value / maxValue) * (height - 10);
    const svg = svgElement('svg', {
      class: 'spark-svg', viewBox: `0 0 ${width} ${height}`, preserveAspectRatio: 'none',
      role: 'img', 'aria-label': options.summary,
    });
    const xy = points.map((point) => ({ x: x(point.time), y: y(point.value) }));
    svg.append(svgElement('path', { class: 'spark-line', d: smoothPath(xy), fill: 'none', stroke: options.color, 'stroke-width': 1.5, 'stroke-linejoin': 'round', 'vector-effect': 'non-scaling-stroke' }));
    const last = xy[xy.length - 1];
    svg.append(svgElement('circle', { class: 'spark-end', cx: last.x.toFixed(1), cy: last.y.toFixed(1), r: 3, fill: options.color }));
